 ...
 * Can be rendered to the screen by the {@link Drawable#draw(AdvancedCanvas)} method.
...
public class GraphicsPolygonAngle extends GraphicsPolygonChild<Angle> {
	...
	public Angle getShape() {
		...
		Vec2 center = copy.getCenter();
		// Get the vertices
		List<Vertex> verts = copy.getVertices();
		// Get a vector from the center of this Angle to one of the outer vertices
		Vec2.Mutable seg0 = 
				new Vec2.Mutable(Vec2.sub(verts.get(0).getCenter(), center));
		Vec2.Mutable seg1 = 
				new Vec2.Mutable(Vec2.sub(verts.get(2).getCenter(), center));
		...
		copy.getVertices().get(0).setCenter(Vec2.add(center, seg0));
		copy.getVertices().get(2).setCenter(Vec2.add(center, seg1));
		// Return the copy
		return copy;
	}
	 ...
	 * However, when we draw this object to a {@link AdvancedCanvas}, we want to draw an
	 * {@link Arc}, not an angle. This method derives an arc from the angle shape.
	 * @return the arc
	 */
	public Arc getArcShape() {
		...
		// Lengths of segments
		final float s0Len = adjSegs[0].getLength();
		final float s1Len = adjSegs[1].getLength();
	...
	public void draw(AdvancedCanvas c) {
		...
		arc.draw(c);
//		c.fillEllipse(getShape().getCenter(), Dimension.TEN);
//		c.arc(getArcShape());
	}
}
